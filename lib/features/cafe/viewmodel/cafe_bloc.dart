...
class CafeBloc extends Bloc<CafeEvent, CafeState> {
  ...
  CafeBloc(this._cafeRepository)
    ...
    _setupLocationCallback();
  }
  void _setupLocationCallback() {
    _locationService.onLocationChanged = (lat, lng) {
      add(LocationUpdated(latitude: lat, longitude: lng));
    };
  }
  ...
  Future<void> _onLoadCafesByDistance(
    ...
    if (_locationService.hasLocation) {
      final location = _locationService.getLocationForApi();
      print('Location for API: $location/*lat: ${location['latitude']}, lng: ${location['longitude']}*/');
      final result = await _cafeRepository.getCafesByDistance(
      ...
      print("Result distance from BLOC: $result"); 
      result.fold((failure) => emit(CafeError(failure.message)), (cafes) {
        _nearbyCafes = cafes;
    ...
    Future<void> _onLoadMoreCafes(
      ...
      result.fold((failure) => emit(CafeError(failure.message)), (newCafes) {
    ...
    Future<void> _onLoadMyCafes(
      ...
      result.fold((failure) => emit(CafeOperationError(failure.message)), (
        cafes,
      ) {
        if (cafes.isEmpty) {
          emit(MyCafesEmpty());
